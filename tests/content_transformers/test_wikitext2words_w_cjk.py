...
"""# Noqa
    assert list(wtpp.transform(text)) == \
            ['松山', '城', 'まつ', 'やま', 'じょう', 'は', '爱媛県', '松山市', 'に', 'あっ',
             'た', '日本', 'の', '城', '别名', '金亀', '城', 'きん', 'きじょう', '胜山', '城',
             'かつ', 'やま', 'じょう', '各地', 'の', '松山', '城', 'と', '区别', 'する', 'ため',
             '伊予', '松山', '城', 'と', '唿ば', 'れる', 'こと', 'も', 'ある', 'が', '一般的',
             'に', '松山', '城', 'は', '本城', 'を', '指す', 'こと', 'が', '多い', 'たびたび',
             '放火', '不审火', 'や', '失火', 'に', 'より', '往时', 'の', '建造物', 'を', '焼失',
             'し', 'て', 'いる', '现在', 'は', '城迹', 'の', '主要', '部分', 'が', '公园', 'と',
             'し', 'て', '整备', 'さ', 'れ', '大天', '守', '现存', 'anumber', '天守', 'の',
             'anumber', 'つ', 'を', '含む', 'anumber', '栋', 'の', '现存', '建造物', 'が',
             '国', 'の', '重要', '文化财', 'に', '城郭', '遗构', 'が', '国', 'の', '史迹', 'に',
             '指定', 'さ', 'れ', 'て', 'いる', 'その', 'ほか', '连立', '式', '天守', '群', 'の',
             '小', '天守', '以下', 'anumber', '栋', 'を', 'はじめ', 'と', 'する', 'anumber',
             '栋', '塀', 'を', '含む', 'が', '木造', 'で', '复元', 'さ', 'れ', 'て', 'いる',
             'また', '现存', '十二', '天守', 'と', 'し', 'て', 'は', '最も', '新しい', '城',
             'で', 'ある', '松山市', 'の', '中心', '部', '胜山', '城山', '山顶', 'に', '本丸',
             '西南', '麓', 'に', '二之丸', 'と', '三之丸', 'を', '构える', '平山', '城', 'で',
             'ある', '日本', '三', '大', '平山', '城', 'に', 'も', '数え', 'られる', '山顶',
             'の', '本', '坛', 'に', 'ある', '天守', '大天', '守', 'は', '日本', 'の', 'anumber',
             '箇所', 'に', '现存', 'する', '天守', 'の', '一', 'つ', 'で', 'ある', 'この', '中',
             'で', 'は', '姫路', '城', 'と', '同じく', '连立', '式', 'で', '日本', '三', '大',
             '连立', '式', '平', '山城', 'の', 'anumber', 'つ', 'に', 'も', '数え', 'られる',
             'anumber', '年ごろ', 'まで', 'は', '本丸', '部分', 'に', 'は', 'anumber', '栋',
             'の', '建造物', 'が', '现存', 'し', 'て', 'い', 'た', 'が', 'anumber', '年', 'まで',
             'に', 'anumber', '栋', 'が', '火灾', 'に', 'より', '失わ', 'れ', '现存', '建筑',
             'は', 'anumber', '栋', 'に', 'まで', '减少', 'し', 'た', '建造物', 'の', '现存',
             '数', 'は', '二', '条', '城', '京都府', 'の', 'anumber', '栋', 'に', '次ぐ', 'もの',
             'で', 'ある', '幕末', 'に', '再建', 'さ', 'れ', 'た', '大天', '守', 'ほか', '日本',
             'で', '现存', '数', 'の', '少ない', '望楼', '型', '二', '重', '橹', 'で', 'ある',
             '野原', '橹', '骑马', '橹', 'や', '深', 'さ', 'anumber', 'メートル', 'に', 'およぶ',
             '本丸', 'の', '井戸', 'など', 'が', '保存', 'さ', 'れ', 'て', 'いる', '山顶', 'に',
             '本丸', '南西', '麓', 'に', '二之丸', '続い', 'て', '三之丸', '北麓', 'に', 'は',
             '北曲轮', '南东', '麓', 'に', '东', '曲轮', 'が', 'ある', '三之丸', 'は', '比高',
             'anumber', 'メートル', 'ほど', 'の', '土塁', 'で', '囲み', '北', 'と', '东', 'に',
            '石垣', '造', 'の', '虎口', 'を', '开く', '本丸', 'から', '二之丸', 'に', 'かけ',
             'て', '登り', '石垣', 'を', '筑い', 'て', '囲み', '丘陵', '斜面', 'から', 'の',
             '大手', '城', '道', 'へ', 'の', '侵入', 'を', '防ぐ', '构造', 'と', 'し', 'て',
             'いる', '山顶', 'の', '本丸', '北部', 'に', 'は', '本', '坛', 'と', 'いう', '天守',
             '曲轮', 'を', '持ち', '大天', '守', 'と', '小天', '守', '・', '南', '隅橹', '・',
             '北', '隅橹', 'を', 'anumber', '栋', 'の', '渡橹', '廊下', 'で', '连结', 'し',
             '连立', '式', '天守', 'を', 'なし', 'て', 'いる', '松山', '城', 'の', '中枢', 'は',
             '二の丸', 'で', '藩主', 'の', '生活', 'の', '场', 'で', 'ある', '御殿', 'や', '庭园',
             '茶室', 'など', 'が', 'あっ', 'た', '三の丸', 'に', 'は', '身分', 'の', '高い',
             '家来', 'の', '屋敷', 'が', '建ち并ん', 'で', 'い', 'た', '本丸', 'は', '主に',
             '仓库', 'と', 'し', 'て', '使わ', 'れ', 'て', 'い', 'た', 'anumber', '年', '庆长',
             'anumber', '年', '伊予', '国', '正木', '城', '松前', '城主', 'anumber', '万', '石',
             'の', '大名', 'で', 'あっ', 'た', '加藤', '嘉明', 'が', '関ヶ原', 'の', '戦い',
             'で', 'の', '戦功', 'に', 'より', 'anumber', '万', '石', 'に', '加増', 'さ', 'れ',
             '足立', '重信', 'を', '普请', '奉行', 'に', '任じ', '麓', 'に', '二之丸', '二之丸',
             '史迹', '庭园', 'と', '三之丸', '堀之内', 'を', '有する', '平山', '城', 'の', '筑城',
             'に', '着手', 'し', 'た']
...
"""# Noqa
    assert list(wtpp.transform(text)) == \
        ['西沟村', '山西省', '平顺县', '西沟', '乡', '的', '一个', '行政村', '西沟', '全村',
         ...
         '开馆', '现', '展馆', '系统地', '展示', '了', '全国', '着名', '劳模', '李顺达', '申纪兰',
